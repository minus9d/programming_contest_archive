// 最初に提出したこのバージョンで正解だった…。
// コラム全体の入れ替えと、ある行での2つの数値の入れ替えのどちらを
// 先にやってもいいことの証明：
// 012という3つの数値の入れ替えを考える。コラム全体の入れ替えは、真ん中と右端に行うものとする。
// (i)先にコラムの入れ替えをする場合
//   まず012 -> 021と入れ替えられる。
//   これに、2つの数値の入れ替えを0回または1回行うと、021, 201, 012, 120のいずれかが得られる。
// (ii)後でコラムの入れ替えをする場合
//   012に、2つの数値の入れ替えを0回または1回行うと、012, 102, 021, 210のいずれかが得られる。
//   コラムを入れ替えて、021, 120, 012, 201が得られる.
// (i)と(ii)で得られるセットは同じ。
