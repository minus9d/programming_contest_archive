...
int main(void)
{
    ...
    REP(m, M) {
        int u, v; cin >> u >> v; u--; v--;
        dij.add_dir_edge(u, v, 1);
        dij.add_dir_edge(v + N, u + N, 1);
        dij.add_dir_edge(v, u + N, X +1);
        dij.add_dir_edge(u + N, v, X +1);
    }

    auto ret = dij.shortest_path(0);
    cout << min(ret[N - 1], ret[N * 2 - 1]) << endl;
