...
bool is_a_same(vector<pair<ll, ll>>& A) {
    FOR(i, 1, SIZE(A)) {
...
bool is_a_alt(vector<pair<ll, ll>>& A) {
    int plus = 0;
    int minus = 0;
    REP(i, SIZE(A)) {
        if (A[i].second > 0) plus++;
        else minus++;
    }
    return abs(plus - minus) <= 1;
}
...
int main(void)
{
    ...
    REP(test, T) {
        ...
        if (A[0].first == A.back().first) {
            cout << ((is_a_same(A) or is_a_alt(A)) ? "Yes": "No") << endl;
        }
        else {
            bool ok = true;
            FOR(n, 2, N) {
