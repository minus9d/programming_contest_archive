...
// https://drken1215.hatenablog.com/entry/2023/11/18/030700
// Lazy Segment Tree
...
int main(void)
{
    ...
    REP(i, SIZE(S)) {
        auto ch = (i % 2) ? '0' : '1';
        S2.push_back((ch == S[i]) ? '0' : '1');
    }

    vector<Node> A(N);
    for (int i = 0; i < N; ++i) {
        if (S2[i] == '1') A[i] = Node{0, 1, 0};
        else A[i] = Node{1, 0, 0};
    }
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            Node res = seg.prod(l, r);
            // cout << "res:" << res.zero << "," << res.one << endl;
            auto len = r - l;
            cout << ((res.zero == len or res.one == len) ? "Yes" : "No") << endl;
        }
    }
