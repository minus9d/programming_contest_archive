...
vector<ll> reach;
ll count_for_id(int v, int id_size, vector<int>& A, int N) {
...
ll get_ans(int v, vector<int>& A, int N) {

    if (reach[v] != -1) return reach[v];
    else {
        ll tmp = 1 + get_ans(A[v], A, N);
        reach[v] = tmp;
        return tmp;
    }
}
...
int main(void)
{
    ...
    REP(v, N) {
        int id = uf.find(v);
        if (seen_id.count(id)) continue;

        int id_size = uf.vertex_size(v);
        ans += count_for_id(v, id_size, A, N);
    ...
    cout << accumulate(ALL(reach), 0LL) << endl;
    return 0;
}
